...
void erosion(OCTET *ImgIn,OCTET *ImgOut,int nH, int nW){
	...
	for (int i=0; i < nH; i++)//j=0 et j=nW-1
	{
	  if(ImgIn[i*nW]==255){//j=0
	   ...
	   if(ImgIn[(i+1)*nW+nW]==255){//j=nW-1

	   	if(i<nH){
	   		ImgOut[(i+1)*nW+nW]=255;
...
char cNomImgLue[250], cNomImgEcrite[250];
  ...
  if (argc != 3) 
   ...
   for (int i=0; i < nH-1; i++)
	...
	ouverture(ImgIn,ImgInt,nH,nW, nTaille);
   fermeture(ImgInt,ImgOut,nH,nW, nTaille);
